...
# Suppress benign "Session with id ... is already connected! Reconnecting" warnings on client reconnects
logging.getLogger('streamlit.runtime.websocket_session_manager').setLevel(logging.ERROR)
...
page_market_watch = st.Page('apps/screener/market_watch.py', title='Market Heatmap', icon='📡')
...
with st.sidebar:
    ...
    # Blog clicks are attributed on the blog side via UTM params; a client-side
    # fetch back to Streamlit never reaches Python, so it cannot be logged here.
    st.html(
        'Read the Blog!<br/>'
        '<a href="https://blog.panendividen.com?utm_source=pd_web&amp;utm_medium=sidebar" target="_blank" '
        'rel="noreferrer" '
